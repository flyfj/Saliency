...
public:
    ...
    // That is if pixel1 and pixel2 are neighbors, call  either setNeighbors(pixel1,pixel2,weight) 
    ...
    // Returns 2 if current optimization algorithm does not check the energy 
...
// Data costs can be specified either by an array or by a pointer to a function 
...
// are specially varying weights use constructor SmoothnessCost(V,hWeights,vWeights)
