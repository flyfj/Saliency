...
namespace objectproposal
{
	...
	bool ObjSegmentRanker::RankSegments(const Mat& cimg, const Mat& dmap, const vector<SuperPixel>& sps, SegmentRankType rtype, vector<int>& orded_sp_ids)
	{
	...
	bool ObjSegmentRanker::RankSegmentsByCC(const Mat& cimg, const vector<SuperPixel>& sps, vector<int>& orded_sp_ids)
	{
	...
	bool ObjSegmentRanker::ComputeSegmentRankFeature(const Mat& cimg, const Mat& dmap, SuperPixel& sp, Mat& feat)
	{
		vector<float> vals;
		// geometric features
	...
	bool ObjSegmentRanker::LearnObjectPredictorFromNYUDepth()
	{
